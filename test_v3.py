...
def insert_data():  # sourcery skip: remove-unused-enumerate
    with open(file=r"ports_list/ports_list.csv", mode="r") as port_list:
...
def expand_all():
    for children in scan_result_tree.get_children():
        scan_result_tree.item(item=children, open=True)


expand_button = CTkButton(
    master=bottom_frame,
    text="Expand",
    text_color="#000000",
    width=150,
    ...
    fg_color="#CD6464",
    corner_radius=50,
    command=lambda: expand_all(),
)
expand_button.grid(row=0, column=0, padx=15, pady=(0, 15), sticky="w")


def collapse_all():
    for children in scan_result_tree.get_children():
        scan_result_tree.item(item=children, open=False)


collapse_button = CTkButton(
    master=bottom_frame,
    text="Collapse",
    text_color="#000000",
    width=150,
    ...
    fg_color="#CD6464",
    corner_radius=50,
    command=lambda: collapse_all(),
)
collapse_button.grid(row=0, column=1, padx=15, pady=(0, 15), sticky="w")

clear_treeview_button = CTkButton(
    master=bottom_frame,
    text="Clear",
    text_color="#000000",
    width=150,
    ...
    fg_color="#CD6464",
    corner_radius=50,
    command=lambda: scan_result_tree.delete(*scan_result_tree.get_children()),
)
clear_treeview_button.grid(row=0, column=2, padx=15, pady=(0, 15), sticky="w")
